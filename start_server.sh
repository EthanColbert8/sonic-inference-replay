...
TRITON_CONTAINER="/depot/cms/users/colberte/SONIC/triton_python_cuda.sif"
...
apptainer run --nv -B "${MODEL_FOLDER}:/models" -B "${DUMP_FOLDER}:/dumps" \
    $TRITON_CONTAINER tritonserver --model-repository=/models --model-config-name=dump_always
